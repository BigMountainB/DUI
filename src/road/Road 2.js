...
export class Road {
  ...
  /** Paint overpass structures (wildlife crossing + I-405 freeway).
   *  Each overpass is a short run of `seg.overpass=true` segments.
   *  Renders an overhead concrete beam spanning above the road, with
   *  decoration on top (vegetation for wildlife, road + guardrails
   *  for freeway).  Near-edge of the beam projects to the near-side
   *  of the overpass, far-edge to the far-side, so the structure has
   *  proper depth. */
  _drawOverpasses(g, drawn) {
    ...
    for (const span of spans) {
      ...
      const far  = span.far;
      // Beam dimensions in screen pixels — wider than the road so it
      // reads as a structure spanning above multiple lanes.
      const beamHeightNear = 38;      // pixels of beam height at near edge
      const beamHeightFar  = 14;
      const overhangNear   = 60;      // beam extends past road edge
      const overhangFar    = 14;
      const liftNear       = 70;      // beam underside above road deck
      const liftFar        = 28;
      // Near edge (closer to camera) — wider, taller beam.
      const nxL = near.screenX - near.screenW - overhangNear;
      const nxR = near.screenX + near.screenW + overhangNear;
      const nyBot = near.screenY - liftNear;
      const nyTop = nyBot - beamHeightNear;
      // Far edge — narrower, thinner.
      const fxL = far.screenX  - far.screenW  - overhangFar;
      const fxR = far.screenX  + far.screenW  + overhangFar;
      const fyBot = far.screenY  - liftFar;
      const fyTop = fyBot - beamHeightFar;

      // ── Beam underside (concrete) ────────────────────────────────
      // Trapezoid: far edge thinner, near edge thicker — reads as the
      // underside of an overhead structure receding into the distance.
      fillTrap(g, 0x4A4640,
        fxL,  fyBot, fxR,  fyBot,
        nxR,  nyBot, nxL,  nyBot);
      // Vertical "support pier" on each side at the near edge so the
      // structure doesn't read as floating.
      g.fillStyle(0x3A3632, 1);
      const pierW = 14;
      g.fillRect(nxL - pierW, nyBot, pierW, near.screenY - nyBot);
      g.fillRect(nxR,         nyBot, pierW, near.screenY - nyBot);
      // ── Beam side face — front of the structure (the bit the
      // player sees as they approach). ─────────────────────────────
      fillTrap(g, 0x6E6A60,
        fxL,  fyTop, fxR,  fyTop,
        nxR,  nyTop, nxL,  nyTop);
      // Connect side face top to underside via two vertical strips
      // (left + right ends of the front face).
      fillTrap(g, 0x5A5650,
        fxL,  fyTop, nxL,  nyTop,
        nxL,  nyBot, fxL,  fyBot);
      fillTrap(g, 0x5A5650,
      ...
      // ── Top-of-beam decoration ──────────────────────────────────
      if (span.kind === 'wildlife') {
        // Green vegetation strip + tree silhouettes on top of the
        // beam — reads as a forested wildlife crossing.
        fillTrap(g, 0x2E5832,
          fxL, fyTop - 10, fxR, fyTop - 10,
          nxR, nyTop - 18, nxL, nyTop - 18);
        // Sprinkle of trees — alternating heights along the top.
        const treeCount = 7;
        for (let t = 0; t < treeCount; t++) {
          const lerp = t / (treeCount - 1);
          const tx   = fxL + (fxR - fxL) * lerp;
          const ty   = fyTop - 10;
          // Scale by depth — center trees larger than edge trees.
          const treeH = 12 + Math.abs(0.5 - lerp) * -4 + 14;
          g.fillStyle(0x1E3A26, 1);
          g.fillTriangle(tx - 5, ty, tx + 5, ty, tx, ty - treeH);
          g.fillStyle(0x16261E, 0.7);
          g.fillRect(tx - 1, ty - 4, 2, 4);
        }
      } else {
        // Freeway overpass: paint a road surface + guardrail along
        // the top of the beam.
        // Road (asphalt) strip
        fillTrap(g, 0x2A2A2A,
          fxL + 4, fyTop - 8, fxR - 4, fyTop - 8,
          nxR - 8, nyTop - 14, nxL + 8, nyTop - 14);
        // Yellow centerline
        const cxFar  = (fxL + fxR) / 2;
        const cxNear = (nxL + nxR) / 2;
        g.lineStyle(2, 0xFFCC22, 0.85);
        g.beginPath();
        g.moveTo(cxFar,  fyTop - 11);
        g.lineTo(cxNear, nyTop - 14);
        g.strokePath();
        // Guardrails (thin grey bars on each side of the deck)
        g.fillStyle(0x9A9A9A, 1);
        const railH = 3;
        // Front rail at the deck's front edge
        fillTrap(g, 0x9A9A9A,
          fxL + 4, fyTop - 13, fxR - 4, fyTop - 13,
          nxR - 8, nyTop - 18, nxL + 8, nyTop - 18);
        // Tiny "I-405" sign on the side of the beam (left end of front
        // face).  Pure decoration — 1 small black rect with letters.
        const signX = nxL + (nxR - nxL) * 0.35;
        const signY = nyTop + beamHeightNear * 0.40;
        const signW = 56, signH = 22;
        g.fillStyle(0x14543E, 1);
